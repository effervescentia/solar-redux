...
export const coreDate = ({ startTime, tick }: State) =>
  date(startTime, tick);

// tslint:disable-next-line
export const planetDate = ({ startTime, planets: { byId: { [name]: { tick } } } }: State, name: string) =>
  dateInMillis(startTime, tick);

// tslint:disable-next-line
export const planetDates = (state: State) =>
  state.planets.allIds.reduce((dates, id) => Object.assign(dates, { [id]: planetDate(state, id) }), {});
...
export const hasPlanet = ({ planets: { allIds } }: State, name: string) => allIds.includes(name);

export const isPlanetMoving = ({ planets: { byId: { [name]: planet } } }: State, name: string) =>
  !planet.stop;
